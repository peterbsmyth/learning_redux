...
// 26
// the point of normalizing is so that we can handle api calls with different
// response shapes in the same way, in our example here we either get an array
// of todos from fetch todos or a single todo from add todo. Normalizr mutates
// the shape so they're both the same shape, allowing the byId reducer to handle
// both responses without special cases. This is a simple example of a much more
// powerful concept, in more complex apis its way more efficient to use
// normalized responses. see the repo on github:
